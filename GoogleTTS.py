...
Example usage:
    python synthesize_text.py --text "A single wavenet can capture the characteristics of many different speakers with equal fidelity."
    python synthesize_text.py --ssml "<speak>Hello there.</speak>"
	
	python GoogleTTS.py --text "A single wavenet can capture the characteristics of many different speakers with equal fidelity."
...
def synthesize_text(text1):
	"""Synthesizes speech from the input string of text."""
	from google.cloud import texttospeech
	
	client = texttospeech.TextToSpeechClient()
	
	#with open('GoogleTTS.txt', 'r') as f:
	#	f.read(texttospeech.types.SynthesisInput(text=text)) #<----------------
